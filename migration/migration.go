...
func (m *migration) HealthCheck(ctx context.Context) error {
	return m.Batch(ctx, func(client MigrationClient) error {
...
func (m *migration) Batch(ctx context.Context, op func(MigrationClient) error) error {
	...
	for _, client := range m.Clients {
		...
		go func(client MigrationClient) {
			if err := op(client); err != nil {
				m.WithError(err).Error("Client op failed")
				errChan <- err
			}
