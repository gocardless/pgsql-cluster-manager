...
func TestMigrationBatch(t *testing.T) {
	testCases := []struct {
		name       string
		operation  func(MigrationClient) error
		batchError error
	}{
		{
			"when for each client operation is successful returns no error",
			func(_ MigrationClient) error { return nil },
			nil,
		},
		{
			...
			func(failWhenNegative int) func(MigrationClient) error {
				return func(_ MigrationClient) error {
					failWhenNegative--
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			m := NewMigration(DefaultConfig(nil))
			m.Clients = []MigrationClient{nil, nil, nil}

			err := m.Batch(tc.operation)
